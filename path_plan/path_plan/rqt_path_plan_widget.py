...
class RqtPath_planWidget(QWidget):
    ...
    def path_publish(self, save_path):
        actpoint_array = np.array(self.path_cal.actpoint)
        deep_position = np.array(self.path_cal.deep_positionlist)
        thickness = np.array(self.path_cal.depthlist)
        center_point = np.array(self.path_cal.closest_point)
        # 将 actpoint 和 rotation 合并为一个字典
        path_data = {
        ...
        # 保存为 .npy 文件
        np.save(save_path, path_data)
        self.PrintLog("路径信息已保存")
    ...
    def cal_finished(self):
        ...
        self._widget.Len.setText(f" {self.path_cal.Length:.2f}")
