...
class RqtHandeyeCalibratorWidget(QWidget):
    def __init__(self, parent, context):
        ...
        self._node.publish_= self._node.create_publisher(String, 'SystemLog', 10)
    ...
    def shutdown(self):
        if self.msg.data is '标定结果已输出':
            subprocess.Popen(['ros2', 'launch', 'easy_handeye2','publish.launch.py'])
            subprocess.Popen(['ros2', 'launch', 'easy_handeye2','evaluate.launch.py'])
            self.PrintLog('变换已发布')
            self.PrintLog('正在评估转化误差')
