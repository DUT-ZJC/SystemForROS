...
def quaternion_multiply(q1, q2):
    x1, y1, z1, w1 = q1
    x2, y2, z2, w2 = q2
    w, x, y, z = qmult((w1, x1, y1, z1), (w2, x2, y2, z2))
    return x, y, z, w


class CalibrationMovements:
    def __init__(self, node: rclpy.node.Node, move_group_name: str, max_velocity_scaling: float,
    ...
    def set_and_check_starting_position(self):
        ...
        if ret:
            self.node.get_logger().info("Set current pose as home")
            return True
        else:
            self.node.get_logger().err("Can't calibrate from this position!")
    ...
    def select_target_pose(self, i):
        ...
        if 0 <= i < number_of_target_poses:
            self.node.get_logger().info("Selected pose {} for next movement".format(i))
            self.current_pose_index = i
            return True
        else:
            self.node.get_logger().err("Index {} is out of bounds: there are {} target poses".format(i, number_of_target_poses))
            return False
    ...
    def _plan_to_pose(self, pose):
        ...
        if CalibrationMovements._is_crazy_plan(plan, self.fallback_joint_limits):
            self.node.get_logger().warn("Planning failed")
            self.plan = None
            return False
        else:
    ...
    def _compute_poses_around_state(start_pose, angle_delta, translation_delta):
        ...
        for rot in final_rots:
            ...
            crx, cry, crz, crw = quaternion_multiply([ori.x, ori.y, ori.z, ori.w], rot)
            fp.pose.orientation = Quaternion(x=crx, y=cry, z=crz, w=crw)
